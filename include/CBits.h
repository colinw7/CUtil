...
namespace CBits {
  ...
  inline void setBit  (Word &c, Word f) { setFlag  (c, Word(1<<f)); }
  inline void resetBit(Word &c, Word f) { resetFlag(c, Word(1<<f)); }
  inline bool testBit (Word c, Word f) { return testFlag(c, Word(1<<f)); }
  ...
  inline Word hiWord(DWord c) { return Word((c & 0xff00) >> 8); }
  inline Word loWord(DWord c) { return Word( c & 0x00ff      ); }

  inline DWord dWord(Word hi, Word lo) { return DWord((hi << 8) | lo); }
}
